...
app.get('/team_stats', function (req, res) {
	...
	var query2 = 'select count(*) from football_games where home_score < visitor_score;'; // loss total
	var query3 = 'select count(*) from football_games where home_score > visitor_score;'; // win total
	db.task('get-everything', task => {
		return task.batch([
			task.any(query1),
			task.any(query2),
		...
		.then(info => {
			res.render('pages/team_stats', {
				...
				data: info[0],
				data_2: info[1][0].count,
				data_3: info[2][0].count
			})
		})
...
app.get('/player_info/post', function (req, res) {
	...
	var query3 = "select CAST(count(*) as INTEGER) from football_games where (select id from football_players where id = " + playerID + ") = ANY(players);"
	...
	db.task('get-everything', task => {
		...
		.then(info => {
			res.render('pages/player_info', {
				...
				year: info[1][0].year,
				gplayed: info[2][0].count,
				pyards: info[1][0].passing_yards,
				ryards: info[1][0].rushing_yards,
				recyards: info[1][0].receiving_yards,
