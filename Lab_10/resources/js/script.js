...
$(document).ready(function() {
  var url ='https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=40.014984,-105.270546&forecast_days=5'; //Place your weatherstack API Call Here - access_key to be used: 5bc82451636190abd9d7afe6fe9b20b5

  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    var image_today = data.current.weather_icons;
    document.getElementById("image_today").src=image_today;
    document.getElementById("heading").innerHTML="Weather forecast for "+data.location.name;
    document.getElementById("temp_today").innerHTML=getFarenheitTemp(data.current.temperature)+"° F";
    document.getElementById("thermometer_inner").style.height=String(getFarenheitTemp(data.current.temperature))+"%";
    document.getElementById("precip_today").innerHTML=data.current.percip+"%";
    ...
    for(i=0;i<5;i++){
      var k = getKey(i);
      var d = dayOfWeek;
      var tHigh= getFarenheitTemp(k.maxtemp);
      var tLow= getFarenheitTemp(k.mintemp);
      var dawn = k.astro.sunrise;
      var twi = k.astro.sunset;
      var card = `<div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title">${d}</h5><p class="card-text">High:${tHigh}<br>Low:${tLow}<br>Sunrise:${dawn}<br>Sunset:${twi}</p></div></div></div>`
    ...
    */
    ...
    /*
      ...
      <div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title"><!-- List Day of the Week Here --></h5><p class="card-text">High:<!--List Temperature High --> <br>Low: <!-- List Temperature Low --><br>Sunrise: <!-- List Time of Sunrise --><br>Sunset: <!-- List Time of Sunset --></p></div></div></div>
